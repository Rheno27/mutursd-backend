...
type CountChart = { labels: string[]; data: number[] };
type RawDistinctRow = { idPasien: string | number };
type RawBioRow = {
...
function buildCountChart(values: Array<string | null | undefined>): CountChart {
  ...
  for (const value of values) {
    const label =
      value && String(value).trim() !== "" ? String(value) : "Tidak diketahui";
    map.set(label, (map.get(label) ?? 0) + 1);
  }
  return { labels: Array.from(map.keys()), data: Array.from(map.values()) };
}
function buildCountChartFromRows(
  rows: Array<Record<string, any>>,
  key: string,
): CountChart {
  return buildCountChart(
    rows.map((row) => row[key] as string | null | undefined),
  );
}

export async function getSkmHasilHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    const distinctRespondentRows = (await AppDataSource.getRepository(
      JawabanEntity,
    )
    ...
    const respondentIds = distinctRespondentRows.map((row) =>
      String(row.idPasien),
    );
    ...
    const chartJenisKelamin = buildCountChartFromRows(
      biodataRows as any[],
      "jenisKelamin",
    );
    const chartPendidikan = buildCountChartFromRows(
      biodataRows as any[],
      "pendidikan",
    );
    const chartPekerjaan = buildCountChartFromRows(
      biodataRows as any[],
      "pekerjaan",
    );
    const chartRuangan = buildCountChartFromRows(
      biodataRows as any[],
      "namaRuangan",
    );
    ...
    for (const row of allQuestionRows as any[]) {
      ...
      questionMap
        .get(key)!
        .pilihan.push({ idPilihan: row.idPilihan, pilihan: row.pilihan });
    }
    ...
    for (const row of allQuestionRows as any[]) {
      ...
      if (!surveyCountMap.has(key))
        surveyCountMap.set(key, { labels: [], data: [] });
    }
    ...
    const allSurveyCharts = Array.from(questionMap.values()).map((question) => {
      const chart = surveyCountMap.get(String(question.idPertanyaan)) ?? {
        labels: [],
        data: [],
      };
