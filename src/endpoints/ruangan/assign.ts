...
function toBoolean(value: unknown): boolean {
  return (
    value === true || value === "true" || value === "t" || Number(value) === 1
  );
}

export async function assignRuanganHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      const indicatorExists = await indicatorRepo
        ...
        .where("im.id_indikator = :idIndikator", {
          idIndikator: idIndikatorBaru,
        })
        .getRawOne();
      ...
      const indikatorRuanganRepo = manager.getRepository(
      ...
      if (existingRow) {
        // PostgreSQL: use $1 placeholder style via TypeORM query
        await manager.query(
          "UPDATE indikator_ruangan SET active = true WHERE id_indikator_ruangan = $1",
          [existingRow.idIndikatorRuangan],
        );
      } else {
        await manager.query(
