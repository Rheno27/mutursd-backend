...
export async function deactivateRuanganHandler(
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    await AppDataSource.transaction(async (manager) => {
      ...
      const existingRows = await manager.query(
        'SELECT id_indikator_ruangan AS "idIndikatorRuangan" FROM indikator_ruangan WHERE id_indikator_ruangan = $1 LIMIT 1',
        [idIndikatorRuangan],
      );
